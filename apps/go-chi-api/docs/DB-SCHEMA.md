...
**Status:** empat file migrasi [task 01](tasks/01-database-migrations.md) sudah
ada di `db/migrations/`. Checklist task 01 mencatat uji migrasi terhadap
...
`UNIQUE (refresh_token_hash)` mencegah hash token yang sama tersimpan di dua
baris. Pemakaian refresh token sekali pakai ditegakkan oleh logika rotasi dan
`revoked_at` di aplikasi, bukan oleh constraint ini.
...
| `completed_at` | `timestamptz` | ya | — | aplikasi mengisinya saat `status` jadi `done`; migrasi tidak memaksa hubungan ini |
| `created_at` | `timestamptz` | tidak | `now()` | |
| `updated_at` | `timestamptz` | tidak | `now()` | |
...
| `sessions_refresh_token_hash_key` | `sessions` | `(refresh_token_hash)` | unique | lookup saat refresh dan mencegah hash token duplikat |
| `sessions_user_id_idx` | `sessions` | `(user_id)` | btree | mencari seluruh session user, misalnya saat token yang dicabut dipakai lagi |
...
- Kolom FK `todos.user_id` dan `todos.list_id` tidak punya index tunggal
  ...
  dan `todos_list_created_id_idx`. Index ini tersedia untuk query scoping dan
  pencarian baris saat `ON DELETE CASCADE`; Postgres tetap memilih rencana
  eksekusi berdasarkan ukuran tabel dan statistik.
...
- **`todos.user_id` harus sama dengan pemilik list tujuan saat todo dipindah.**
  Database tidak menegakkan bahwa `todos.user_id` sama dengan `lists.user_id`
  dari `todos.list_id`. Usecase pindah list harus memastikan list tujuan milik
  user yang sama dan menjaga nilai ini tetap konsisten (ADR-011); wajib diuji.
