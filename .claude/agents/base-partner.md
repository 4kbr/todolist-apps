...
Per catatan terakhir: **task 00 (foundation) selesai**, berikutnya
...
- Target migrasi bernama `migrate-up`, `migrate-down`, `migrate-status`,
  `migrate-create`, bukan `db-*`. Referensi `db-up` di dokumen lama sudah usang.
